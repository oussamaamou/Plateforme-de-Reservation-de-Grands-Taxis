            ...
            <div id="ctnrcsltion" class="hidden fixed left-[32rem] top-[-1rem] flex flex-col items-center justify-center px-8 py-8 mx-auto md:h-screen lg:py-0">
                <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                        ...
                        <form class="space-y-4 md:space-y-6" action="{{route('create.reservation')}}" method="POST">
                            @csrf

                            <input type="hidden" name="passager_id" value="{{ Auth::user()->id }}">
                            <input type="hidden" id="chauffeur_id" name="chauffeur_id" value="">
            ...
            <div class="mt-[4rem] ml-[12rem] bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-4xl w-full p-8 transition-all duration-300 animate-fade-in pt-[3.5rem]">
                <div class="flex flex-col md:flex-row">
                    <div class="md:w-1/3 text-center mb-8 md:mb-0">
                        <img src="{{ $chauffeur->photo }}" alt="Profile Picture" class="rounded-full w-48 h-48 mx-auto mb-4 border-4 border-amber-500 transition-transform duration-300 hover:scale-105">
                        ...
                        <button onclick="addReservationForm({{ $chauffeur->id }})" class="mt-4 font-medium text-white px-4 py-2 rounded-lg bg-gradient-to-r from-amber-400 via-amber-500 to-amber-600 hover:bg-gradient-to-br transition-colors duration-300">Reserver</button>
                    </div>
...
function addReservationForm(chauffeur_id) {
